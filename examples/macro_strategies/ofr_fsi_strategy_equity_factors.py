...
def get_data():
    ...
    :return: DataFrame
    """
    low_vol_df = pd.read_excel(r"C:\Users\gafza\OneDrive\Dokument\Excel\Data\QIS\backtest_qis\s&p_500_low_volatility_20230123.xlsx",
                               index_col=0)
    low_vol_df = low_vol_df[['LOW_VOL_EQW']].copy()
    mom_df = pd.read_excel(r"C:\Users\gafza\OneDrive\Dokument\Excel\Data\QIS\backtest_qis\s&p_500_momentum_20230123.xlsx",
                           index_col=0)
    mom_df = mom_df[['MOM_EQW']].copy()
    df = low_vol_df.join(mom_df)
...
def main():
    ...
    strat.annual_fee = 0.00
    strat.transaction_costs = 0.0015
    ...
    strat.weight = CustomFixedWeight(weight_map={'MOM_EQW': 0.5, 'LOW_VOL_EQW': 0.5})
